...
def upload_resume():
    if request.method == "POST":
        ...
        if file and allowed_file(file.filename):
            ...
            data = ResumeParser(f"./resources/{file.filename}").get_extracted_data()
            global data_list
...
def print_shit():
    if request.method == "POST":
        ...
        for search_keys in super_var_unique:
            browser_sim(search_keys, driver_loc, "./resources/")
            searches = search_keys
            search_linkedin(search_keys, "Singapore", driver_loc, "./resources/")
...
def download():
    ...
    with zipfile.ZipFile(
        "resources.zip", mode="w", compression=compression_type
    ) as archive:
        for file_path in directory.rglob("*"):
            archive.write(file_path, arcname=file_path.relative_to(directory))
    return send_file(
        "./resources.zip", attachment_filename="resources.zip", as_attachment=True
    )
